...
class App(tk.Frame):
    def __init__(self, master):
        ...
        tk.Label(f1, text='Username whose followers/friends you hope to learn:' 
        ...
        ## frame 2 buttons
        fb2 = tk.Frame(self)
        fb2.pack(padx=60, pady=(0,30), anchor='e')
        ...
        self.stb5 = tk.Button(fb2, text='Stop & Quit', height=1, width=7, command=self.click_cancel)
        self.stb5.pack(side='right')
    ...
    def hover_on3(self, event=None):
        self.label.config(text="Click to get information about these ids in \'.json\' format")

    def hover_on4(self, event=None):
        self.label.config(text="Click to get information about these ids in \'.csv\' format")
...
######
    ...
    def midthread(self, event=None):
        global start_bt_ms
        start_bt_ms = "Started getting " + self.followFriend + " of account \'" + self.l1.get() + "\'" 
        self.label.config(text=start_bt_ms)
